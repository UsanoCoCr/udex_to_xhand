...
int main() {
  ...
  if (ret) {
    ...
    {
      auto ret = xhand_control.get_serial_number(hand_list[0]);
      if (ret.first)
        std::cout << "serial number: " << ret.second << std::endl;
      else
        std::cout << "get serial number failed: " << ret.first.error_code
                  << " " << ret.first.error_message << std::endl;
    }
...

    {
      printf("***********************\n");
      // read firmware state
      auto ret = xhand_control.read_firmware_state(hand_list[0], 11);
      if (ret.first) {
        std::cout << " firmware version: " << ret.second << std::endl;
      } else {
        std::cout << "read version failed:" << ret.first.error_code << "  "
                  << ret.first.error_message << std::endl;
      }
...

    {
      for (int i = 0; i < 12; i++) {
        ...
        if (ret.first) {
          std::cout << " joint " << i << " version: " << ret.second
                    << std::endl;
        } else {
          std::cout << "read version failed: " << ret.first.error_code << "  "
                    << ret.first.error_message << std::endl;
    ...
    
    {
      ...
      switch (choice) {
          case 0:
              ...
              break;
          case 3:
              std::cout << "Selected mode: Position Control Mode" << std::endl;
              kp = 225;
              ki = 0;
              kd = 0;
              tor_max = 350;
    ...
    // joint
    // {
    //   std::cout << "Verify or update firmware path before upgrade." << std::endl;
    ...
    //           hand_list[0], i,
    //           "../firmware/jointboard_g0_v1.0.8_20241206185846.bin", 1000);
    //       if (ret) {
    ...
    //       auto ret = xhand_control.upgrade_device(
    //           hand_list[0], i,
    //           "../firmware/fingerboard_g071_v1.0.2_20250108150204.bin", 1000);
    //       if (ret) {
    ...
    //         "../firmware/commboard_v99.3.6_202505291737.bin", 1000);
    //     if (ret) {
    //       std::cout << "upgrade palm successfully" << std::endl;
    //       std::cout << "Upgrade complete. Please restart the program." << std::endl;
    //       return 0;
    //     } else {
  ...
  while (!exit_flag.load()) {
...

    {
      ...
      auto ret = xhand_control.read_state(hand_list[0], false);
      if (ret.first) {
        auto state = ret.second;
        // int finger_id = 0;
        std::cout << "Finger " << finger_id
