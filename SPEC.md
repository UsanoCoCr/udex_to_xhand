...
├── *.py                          # Legacy Python prototype from M0-M4; superseded by src/ after M5b
...
10. **C++ ↔ legacy Python equivalence** — ensure new C++ `joint_mapper` reproduces the Python M4 output for the same `config.yaml` + UDCAP input. Plan: snapshot test using `example.json` (M5b sub-task).
...
| UDP receiver prototype (Python, legacy) | `test.py` / `udcap_receiver.py` |
| Python prototype reference (M0-M4; pre-C++ rewrite) | `main.py`, `udcap_receiver.py`, `joint_mapper.py`, `xhand_driver.py`, `safety.py` |
