...
### Test 3: JSON parse failure (requires LAN, manual test) -- PASSED
...
### Test 4: Real UDCAP standalone (requires UDCAP hardware) -- PASSED
```bash
python udcap_receiver.py --port 9000 --duration 10
```
Expected:
...
### Test 5: Integration with main.py (real UDP, stub XHand) -- PASSED
```bash
...
### Test 6: Ctrl+C clean shutdown -- PASSED
```bash
